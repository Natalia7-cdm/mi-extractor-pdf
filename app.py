...
# 1. Inicializamos la "memoria" del portal
if 'procesado' not in st.session_state:
    st.session_state.procesado = False
    st.session_state.excel_data = None
    st.session_state.total_filas = 0
    
# NUEVO: Memoria para engañar a la cajita del PDF y que se vacíe
if 'uploader_key' not in st.session_state:
    st.session_state.uploader_key = 1

# NUEVO: Le asignamos la clave dinámica a la cajita
...
if uploaded_file is not None:
    ...
    if st.session_state.procesado:
        ...
        if st.button("🔄 Procesar nuevo archivo / Limpiar pantalla"):
            # NUEVO: Al limpiar, borramos los datos y cambiamos la clave de la cajita para forzar su vaciado
            st.session_state.procesado = False
            st.session_state.excel_data = None
            st.session_state.total_filas = 0
            st.session_state.uploader_key += 1 # Cambiamos el ID
            st.rerun()